...
- **Daily check-in**: the account row of Settings → Plugins → DSH CodeBuddy CLI Connect carries a Check in button that calls the CodeBuddy daily check-in endpoint and shows the outcome beside it — success, already checked in today, or the failure reason. CN CodeBuddy accounts only (workbuddy.ai global accounts are told it is unavailable). The check-in reuses the plugin's local sign-in; the browser never sees token material.
- **Status card**: Settings → Plugins → DSH CodeBuddy CLI Connect shows the account, token expiry, remaining credit, and the models currently on a promo. The body is split into three collapsible sections — Available models, Remaining credit, Model offers — collapsed by default with only the account block open; each section keeps a summary beside its title (models offered, credit total, promo count), so the credit total reads without expanding anything.
