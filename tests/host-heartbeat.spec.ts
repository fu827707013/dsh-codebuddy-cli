...
describe('host heartbeat', () => {
  it('writes, reads, and clears a heartbeat under $DSH_HOME', async () => {
    ...
    expect(heartbeat!.package).toBe('dsh-codebuddy-cli')
    expect(heartbeat!.pid).toBe(process.pid)
    expect(typeof heartbeat!.registeredAt).toBe('number')
    expect(heartbeat!.pluginVersion).toBe(CODEBUDDY_CLI_VERSION)

    // The file lives at the expected path.
    expect(codebuddyHostHeartbeatPath()).toBe(join(root, CODEBUDDY_HOST_HEARTBEAT_FILENAME))
  ...
  it('detects a recycled PID as dead (registeredAt after this process started)', async () => {
    ...
    const recycled = {
      version: 1 as const,
      package: 'dsh-codebuddy-cli' as const,
  ...
  it('treats a malformed heartbeat file as absent', async () => {
    ...
    await writeFile(codebuddyHostHeartbeatPath(), '{ not json', 'utf8')
    expect(await readHostHeartbeat()).toBeUndefined()
  })
  ...
  it('rejects a heartbeat with the wrong format version', async () => {
    ...
    await writeFile(
      codebuddyHostHeartbeatPath(),
      JSON.stringify({ version: 99, package: 'dsh-codebuddy-cli', registeredAt: Date.now(), pid: process.pid }),
      'utf8',
    )
    expect(await readHostHeartbeat()).toBeUndefined()
  })
