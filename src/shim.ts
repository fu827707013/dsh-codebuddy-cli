...
import type { CodeBuddyCatalog } from './catalog.ts'
import { hostIsLoopback, originIsLoopback } from './loopback.ts'
import { prepareChatBody, CodeBuddyUpstreamClient, type UpstreamErrorKind } from './upstream.ts'
...
export interface CodeBuddyShim {
...
export interface CodeBuddyShimOptions {
 ...
 */
export function createCodeBuddyShim(options: CodeBuddyShimOptions): CodeBuddyShim {
  ...
  const baseUrl = (): string => {
    ...
    if (address === null || typeof address === 'string') {
      throw new Error('codebuddy shim has no listening address')
    }
  ...
  async function chatCompletions(req: IncomingMessage, res: ServerResponse): Promise<void> {
    ...
    if (!result.ok) {
      writeOpenAIError(
        ...
        result.kind,
        `codebuddy upstream ${result.kind} (http ${result.status}): ${result.message.slice(0, 400)}`,
      )
      return
    }
