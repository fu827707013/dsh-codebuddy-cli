...
export function codebuddyHostHeartbeatPath(): string {
...
export async function writeHostHeartbeat(): Promise<void> {
  const document: CodeBuddyHostHeartbeat = {
...
export async function readHostHeartbeat(): Promise<CodeBuddyHostHeartbeat | undefined> {
  let raw: string
  try {
  ...
  try {
    const parsed = JSON.parse(raw) as Partial<CodeBuddyHostHeartbeat>
    if (
      parsed.version === HEARTBEAT_FORMAT_VERSION
      && parsed.package === 'dsh-codebuddy-cli'
...
export function processStartTimeMs(pid: number): number | undefined {
  try {
    if (process.platform === 'win32') {
      // wmic first (fast, present on Windows 10); Windows 11 24H2+ removed
      // it, so fall back to PowerShell's `Get-Process` query. A wmic ENOENT
      // or unparsable answer both fall through to the fallback.
      try {
        ...
        // wmic missing or failed — use the PowerShell fallback below.
      }
      // `Get-Process` StartTime is a local DateTime; formatted without an
      // offset and parsed by `Date.parse` it resolves against the local
      // clock, matching how `registeredAt` is expressed.
 ...
 */
export function isHeartbeatProcessAlive(heartbeat: CodeBuddyHostHeartbeat): boolean {
  try {
