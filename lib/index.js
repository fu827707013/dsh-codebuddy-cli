...
*/
const CODEBUDDY_IMPORT_ACCOUNT_PATH = "/plugins/dsh-codebuddy-cli/accounts/import";
//#endregion
...
*/
function codeBuddyImportAccountHandler(deps) {
	return async (req, res) => {
		if (req.method !== "POST") {
			json(res, 405, { error: "method not allowed" });
			return;
		...
		try {
			...
			const nicknameHint = typeof parsed.nickname === "string" && parsed.nickname !== "" ? parsed.nickname : void 0;
			const enterpriseHint = typeof parsed.enterpriseId === "string" && parsed.enterpriseId !== "" ? parsed.enterpriseId : void 0;
			const claims = pastedAccess === "" ? void 0 : claimsFromToken(pastedAccess);
			...
			const probeDomain = pastedDomain !== "" ? pastedDomain : regionOf(domainFromClaims(claims) ?? "") === "global" ? domainFromClaims(claims) ?? "" : "";
			const probe = {
				...
				uid: uidFromToken ?? "",
				source: "dsh",
				...enterpriseFromToken !== void 0 ? { enterpriseId: enterpriseFromToken } : {}
			};
			...
			const credential = {
				...
				expiresAtMs: outcome.expiresInSec !== void 0 ? now + outcome.expiresInSec * 1e3 : now + 36e5,
				domain: resolvedDomain,
				uid: uidFromToken ?? "",
				source: "dsh",
				...outcome.refreshExpiresInSec !== void 0 ? { refreshExpiresAtMs: now + outcome.refreshExpiresInSec * 1e3 } : {},
				...enterpriseFromToken !== void 0 ? { enterpriseId: enterpriseFromToken } : {}
			};
			...
			const nickname = nicknameHint ?? nicknameFromToken ?? claimString(freshClaims, "nickname") ?? claimString(freshClaims, "preferred_username");
			const id = generateAccountId();
			const stored = await deps.accountStore.add({
				id,
				uid,
			...
			if (credits !== void 0) {
				const at = Date.now();
				accountCreditsCache.set(id, {
					at,
					payload: {
						credits,
						creditUpdatedAtMs: at
					}
				});
			}
			json(res, 200, {
...
*/
function domainFromClaims(claims) {
...
*/
function plainSettingsSchema(schema) {
	const clone = structuredClone(schema.toJSON());
	for (const node of Object.values(clone.refs ?? {})) if (node !== null && typeof node === "object" && node.meta !== void 0) delete node.meta.volatile;
	return z(clone);
}
/**
...
*/
function resolveSettingsNamespaceCompat(ctx, provider, fallback) {
	if (compatModule.installSettingsSection !== void 0) return fallback;
	const candidate = provider;
	if (candidate.installSection !== void 0) return fallback;
	if (candidate.configure !== void 0) return entryNamespaceOf(ctx) ?? fallback;
	return fallback;
}
/**
...
function installSettingsSectionCompat(ctx, ns, schema, entry, hooks) {
	...
	ctx.inject(["settings"], (sctx) => {
		...
		if (provider.installSection !== void 0) {
			provider.installSection(ctx, ns, plainSettingsSchema(schema), entry, hooks);
			return;
		}
		if (provider.configure === void 0) throw new TypeError("dsh-settings exposes neither installSection nor configure");
		sctx.effect(() => {
...
*/
function settingsCompatNamespace(ctx) {
